...
from model import HWSNet, ACLLoss
import utils
...
class Trainer():
    ...
    def __init__(self, dataset, epochs=10, lr=0.01, momentum=0.8, bz=20, 
                weight_decay=1e-4, lr_gamma=0.8, train_cnn_after=3,
        ...
        self.cnn_lr_gamma=0.1 # scaled lr for pre-trained vgg 
        self.train_cnn_after = train_cnn_after
        self.cnn_lr = lr*self.cnn_lr_gamma
        self.cnn_weight_decay = weight_decay*self.cnn_lr_gamma
        self.best_weight = best_weight
        self.copy_code_flag = copy_code_flag
        self.early_stop_epoch = early_stop_epoch

        self.net = HWSNet().cuda()
        self.cnn_params=[]
        self.new_params=[]
        for name, params in self.net.named_parameters():
            if 'CNN_feats' in name:
                self.cnn_params.append(params)
            else:
                self.new_params.append(params)

        self.optimizer =  torch.optim.Adam([{'params': self.new_params},
                                {'params': self.cnn_params,
                                'lr':self.cnn_lr,
    ...
    def fit(self, train_loader, val_loader, clip_=False):
        ...
        for epoch in range(self.epochs):
            self.net.train()
            cnn_grad = epoch>self.train_cnn_after
            for cnn_param in self.net.CNN_feats.parameters():
                cnn_param.requires_grad = cnn_grad
                # print('CNN pramarmeters trainable:', cnn_grad)

            losses = 0
            metrics = [0]*len(self.metric_list)
            # phar = tqdm(total=len(train_loader.dataset))
            timestamp = time.time()
            print('########## epoch {:0>3d}; \t new_params lr: {:.8f} \t CNN trainable: {} ##########'.format(epoch, 
                    self.optimizer.param_groups[0]["lr"], cnn_grad))
            if cnn_grad: print('CNN lr:  {:.8f}'.format(self.optimizer.param_groups[1]["lr"]))
...
if __name__=="__main__":
        ...
        # train_config: epoch, lr_gamma, train_cnn_after, early_stop_epoch
        data_config = {"EyeTrack":[1000, 0.98, 45, 50],
                     "DReye":[200, 0.95, 28, 30], 
                     "DADA2000":[200, 0.95, 28, 30],
                      "BDDA":[200, 0.95, 28, 30]}
        num_workers= 0
        bz = 4
        torch_seed(2023)
        ...
        for dataset in ["DADA2000"]:
            ...
            Trainer_ = Trainer(dataset=dataset,
                                ...
                                weight_decay=5e-4,
                                bz=bz, 
                                train_cnn_after=configs[2],
                                early_stop_epoch=configs[3],
                                best_weight='runs/DADA2000_2021-11-23_112614/best_weight_DADA2000.pt'
                                )
        ...
        #     Trainer_ = Trainer(dataset=dataset,
        #                         best_weight='./runs/DReye_2021-11-23_101659/best_weight_DReye.pt',
        #                         bz=bz, 
