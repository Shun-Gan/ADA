...
def schedule_mynet(epoch):
    lr = [1e-3, 1e-4, 1e-4, 1e-4, 1e-5, 1e-5, 1e-5, 1e-6, 1e-6,
...
if __name__ == '__main__':
    if mode == 'train':
        ...
        m = SaliencyBranch(input_shape=(16,448, 448, 3), c3d_pretrained=False, branch='image')
        m.summary()
        print("Compiling My_Net")
        # m.compile(Adam(lr=1e-4), loss=[kl_loss, cc_loss, nss_loss], loss_weights=[1, 0.1, 0.1])  #
        m.compile(Adam(lr=1e-3), loss=[kl_loss, kl_loss], loss_weights=[0.7, 0.3])  #
        print("Training My_Net")
        if pre_train:
            m.load_weights(pre_train_path)

        m.fit_generator(generator_data(video_b_s=batch_size, phase_gen='train', source=source), nb_train, epochs=nb_epoch,
                        validation_data=generator_data(video_b_s=batch_size, phase_gen='val', source=source),
                        validation_steps=nb_videos_val,
                        callbacks=[  # EarlyStopping(patience=10),
                            ModelCheckpoint('./models/%s_{epoch:02d}_{val_loss:.4f}.h5'%source, save_best_only=False),
            ...
            for source in ['DADA']:
                ...
                for idx, batch_data in enumerate(test_data):
                    if idx > score_batch[source]:
                        break
                    [I_last, I_fine, I_crop], [M_fine, M_crop, Y_fixs] =batch_data
                    [pred_map, _] = m.predict(x= [I_last, I_fine, I_crop], batch_size=batch_size)
                    # kld_batch = kl_loss(Ymaps, tf.cast(pre_map[-1], tf.float64)) # tf.stack
                    # cc_batch = cc_loss(Ymaps,tf.cast(pre_map[-1], tf.float64))
                    kld_batch = kl_loss(M_fine, tf.cast(pred_map, tf.float64)) # tf.stack
                    cc_batch = cc_loss(M_fine,tf.cast(pred_map, tf.float64))
                    nss_batch = nss_loss(Y_fixs, tf.cast(pred_map, tf.float64))
                    ...
                    pred_np = np.squeeze(pred_map)
                    fix_np = np.squeeze(Y_fixs)
                    auc_sum += auc_judd(pred_np, fix_np)
                    sim_sum += sim(pred_map, M_fine)
                    nss_sum += np.mean(nss_np)
                ...
                output_dict['nss'].append(nss_sum/(idx))
                output_dict['auc_j'].append(auc_sum/(idx))

            output_pd_set = pd.DataFrame(output_dict)
            output_pd_set.to_csv('./%s_dataset_20211210.csv'%weight_source)
            print('write %s_dataset_0.csv'%weight_source)
        output_pd = pd.DataFrame(output_dict)
        output_pd.to_csv('./result_mertrics_0.csv')
