...
class Data_Dataset(Dataset):
    ...
    def load_data(self, dkey, data_file):
        ...
        if read_flag is not None:
            ...
            data = cv2.resize(data, tuple((int(self.img_size[1]/8), int(self.img_size[0]/8))), 
                    interpolation=cv2.INTER_CUBIC)
        else:
            data = cv2.imread(str(data_file))
            data = cv2.resize(data, tuple((int(self.img_size[1]), int(self.img_size[0]))), 
                    interpolation=cv2.INTER_CUBIC)
        if data is None:
            raise FileNotFoundError(data_file)

        # data = cv2.resize(data, tuple(self.img_size), interpolation=cv2.INTER_CUBIC)
