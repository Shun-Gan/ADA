...
class Example_prediction():
    ...
    def read_img_seq(self, imgs):
        ...
        for img_path in imgs:
            ...
            fine_img = img.astype('float32')/255.
            fine_img_seq.append(fine_img.transpose(2, 0, 1))

            coarse_img = cv2.resize(img, (self.img_size[1], self.img_size[0]))
            coarse_img = coarse_img.astype('float32')/255.
            coarse_img_seq.append(coarse_img.transpose(2, 0, 1))
        return coarse_img_seq, fine_img_seq
    ...
    def predict_example(self, root_path):
        ...
        for img_folder in img_folders:
            ...
            coarse_img_seq, fine_img_seq = self.read_img_seq(imgs) # [c, h, w]*num
            
            self.net.eval()
            for idx, coarse_img in enumerate(coarse_img_seq):
                coarse_img = torch.from_numpy(coarse_img)
                coarse_img = torch.unsqueeze(coarse_img, dim=0) # [1, c, num, h, w]
                fine_img = torch.from_numpy(fine_img_seq[idx])
                fine_img = torch.unsqueeze(fine_img, dim=0)

                logits = self.net(fine_img.cuda(), coarse_img.cuda())
