...
class Trainer(utils.KwConfigClass):
    ...
    phases = ('train', 'valid')
    # all_data_sources = ('DHF1K', 'Hollywood', 'UCFSports', 'SALICON', 'DADA2000', "DReye")
    all_data_sources = ("DReye", 'DADA2000', 'DT16', 'BDDA')
    ...
    def __init__(self,
                 ...
                 grad_clip=2.,
                 loss_metrics=('kld', 'cc','sim'), ### delete nss
                 loss_weights=(0.8, -0.1,-0.1),  ### (1, -0.1, -0.1)
                 data_sources=('DReye','DADA2000', 'DT16', 'BDDA'),
                 batch_size=4,
                 data_cfg=None,
        ...
        self.dada2000_batch_size = dada2000_batch_size
        self.dada2000_cfg = dada2000_cfg or {}
        self.dreye_batch_size = dreye_batch_size
        self.dreye_cfg = dreye_cfg or {}
        self.dt16_batch_size = dt16_batch_size
        self.dt16_cfg = dt16_cfg or {}
        self.bdda_batch_size = bdda_batch_size
        self.bdda_cfg = bdda_cfg or {}
        ...
        if debug:
            ...
            self.eval_subeset = 1.
            self.dada2000_cfg.update({'subset': 0.02})
            self.dreye_cfg.update({'subset': 0.02})
            self.dt16_cfg.update({'subset': 0.2})
            self.bdda_cfg.update({'subset': 0.02})
        ...
        ###
        self._dada2000_datasets = {}
        self._dada2000_dataloaders ={}
        self._dreye_datasets = {}
        self._dreye_dataloaders = {}
        self._dt16_datasets = {}
        self._dt16_dataloaders = {}
        self._bdda_datasets = {}
        self._bdda_dataloaders = {}
    ...
    def fit_phase(self):
        ...
        for src in sources_eval:
            ...
            ### DADA2000
            if src == "DADA2000" and self.phase == 'valid' and\
    ...
    def loss_sequences(pred_seq, sal_seq, fix_seq, metrics):
        ...
        for this_metric in metrics:
            ...
            if this_metric == 'new_kld':
                losses.append(utils.new_kld(pred_seq.exp(), sal_seq))
            if this_metric == 'new_cc':
                losses.append(utils.new_cc(pred_seq.exp(), sal_seq))
            if this_metric == 'sim':
                losses.append(utils.new_sim(pred_seq.exp(), sal_seq))
        return losses
    ...
    def run_inference(self, source, vid_nr, dataset=None, phase=None,
        ...
        if metrics is not None:
            ...
            fix_seq = torch.full(results_size, 0, dtype=torch.bool) # 修改
        else:
            ...
            def other_maps():
                ...
                while True:
                    ...
                    for map_idx, vid_nr in enumerate(video_nrs):
                        ...
                        this_this_map = this_this_map.astype(np.uint8) # 修改：增加
    ...
    def generate_predictions_from_path(
            self, folder_path, is_video, source=None, load_weights=True,train_id=None,
            **kwargs):
        ...
        if load_weights:
            ...
            # the last epoch
            try:
                self.model.load_best_trained_weights()
                print('Best weights loaded')
            except FileNotFoundError:
                print('No best weights found')
                self.model.load_last_chkpnt(self.train_dir)
    ...
    def get_dataloader(self, phase, source="DHF1K"):
        ...
        if phase not in self._dataloaders[source]:
            ...
            ### 
            elif source in ("Hollywood", "UCFSports","DADA2000", 'DReye', 'DT16', 'BDDA'):
    ...
    def export_scalars(self, suffix=''):
        ...
        with open(export_dir / f'new_all_scalars{suffix}.json', 'w') as f:
